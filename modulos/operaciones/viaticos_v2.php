...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['guardar_viatico'])) {
        ...
        header('Location: viaticos_v2.php?' . http_build_query([
...
// Funciones auxiliares locales para validaciones dinámicas basadas en BD
/**
 * Obtiene todas las reglas de viáticos activas de la base de datos
 */
function obtenerReglasViaticosActivasV2() {
    global $conn;
    try {
        $stmt = $conn->prepare("SELECT sucursal_codigo, tipo, dias_semana, hora_inicio, hora_fin FROM viatico_reglas_sucursal WHERE activa = 1");
        $stmt->execute();
        return $stmt->fetchAll(PDO::FETCH_ASSOC);
    } catch (PDOException $e) {
        error_log("Error al obtener reglas de viáticos: " . $e->getMessage());
        return [];
    }
}

/**
 * Verifica si un día de semana (1-7) aplica para una regla dada
 */
function aplicaReglaFechaV2($regla, $fecha) {
    if (empty($regla['dias_semana'])) {
        return false;
    }
    $diaSemana = (int) date('N', strtotime($fecha));
    $diasPermitidos = array_map('intval', explode(',', $regla['dias_semana']));
    ...
    try {
        $sql = "
            SELECT 
                ...
                r.tipo as tipo, 
                CASE 
                    WHEN r.tipo = 'Diurno' THEN d.viatico_diurno
                    ELSE d.viatico_nocturno
                END as cantidad,
                CONCAT(
                    'Turno marcado (', r.tipo, ')<div class=\"hora-viatico\">',
                    CASE 
                ...
                r.dias_semana as dias_semana_regla,
                v.creado_por,
                v.fecha_creacion,
                v.actualizado_por,
            ...
            JOIN viatico_reglas_sucursal r ON m.sucursal_codigo = r.sucursal_codigo AND r.activa = 1
            ...
            AND (
                -- Evaluar dinámicamente si califica en base a las horas de la regla
                (r.hora_fin IS NULL AND (
                    (m.hora_salida >= r.hora_inicio AND m.hora_salida IS NOT NULL) OR
                    (m.hora_ingreso >= r.hora_inicio AND m.hora_ingreso IS NOT NULL) OR
                    (m.hora_ingreso IS NULL AND m.hora_salida IS NOT NULL AND m.hora_salida >= r.hora_inicio) OR
                    (m.hora_salida IS NULL AND m.hora_ingreso IS NOT NULL AND m.hora_ingreso >= r.hora_inicio)
                ))
                OR 
                (r.hora_fin IS NOT NULL AND (
                    (m.hora_ingreso BETWEEN r.hora_inicio AND r.hora_fin) OR
                    (m.hora_salida BETWEEN r.hora_inicio AND r.hora_fin)
                ))
            )
            AND (m.hora_ingreso IS NOT NULL OR m.hora_salida IS NOT NULL)
            -- Excluir operarios asignados principalmente a sucursales que no tienen reglas de viáticos configuradas
            AND m.CodOperario NOT IN (
                SELECT DISTINCT anc_ex.CodOperario
                FROM AsignacionNivelesCargos anc_ex
                WHERE anc_ex.Sucursal NOT IN (
                    SELECT DISTINCT sucursal_codigo FROM viatico_reglas_sucursal WHERE activa = 1
                )
        ...
        // Filtrar por los días permitidos en la regla de viáticos de la sucursal
        $resultadosFiltrados = [];
        foreach ($resultados as $viatico) {
            $fecha = $viatico['fecha'];
            $diasSemanaRegla = $viatico['dias_semana_regla'];
            
            if (!empty($diasSemanaRegla)) {
                $diaSemana = (int) date('N', strtotime($fecha)); // 1=lunes, 7=domingo
                $diasPermitidos = array_map('intval', explode(',', $diasSemanaRegla));
...
function guardarViaticoAutomatico($codOperario, $fecha, $sucursalCodigo, $cantidad, $observaciones, $fechaPago = null, $tipo = 'Nocturno') {
    ...
    try {
        // Obtener la regla activa para la sucursal y tipo indicados
        $stmtRegla = $conn->prepare("
            SELECT dias_semana, hora_inicio, hora_fin 
            FROM viatico_reglas_sucursal 
            WHERE sucursal_codigo = ? AND tipo = ? AND activa = 1 
            LIMIT 1
        ");
        $stmtRegla->execute([$sucursalCodigo, $tipo]);
        $regla = $stmtRegla->fetch();
        if (!$regla) {
            throw new Exception("La sucursal $sucursalCodigo no aplica para viáticos tipo $tipo");
        }
        
        // Verificar si la fecha seleccionada aplica según el día de la semana de la regla
        if (!aplicaReglaFechaV2($regla, $fecha)) {
            throw new Exception("Los viáticos tipo $tipo no aplican para la fecha $fecha en esta sucursal");
        }
        
        // Obtener el monto correcto configurado en el departamento de la sucursal
        $stmtMonto = $conn->prepare("
            SELECT d.viatico_nocturno, d.viatico_diurno 
            FROM sucursales s
            JOIN departamentos d ON s.cod_departamento = d.codigo
            WHERE s.codigo = ? 
            LIMIT 1
        ");
        $stmtMonto->execute([$sucursalCodigo]);
        $montosDepto = $stmtMonto->fetch();
        if (!$montosDepto) {
            throw new Exception("No se encontró información del departamento para la sucursal $sucursalCodigo");
        }
        
        $montoCorrecto = ($tipo === 'Diurno') ? ($montosDepto['viatico_diurno'] ?? 0) : ($montosDepto['viatico_nocturno'] ?? 0);
        if (floatval($montoCorrecto) != floatval($cantidad)) {
            throw new Exception("El monto ($cantidad) no coincide con el configurado para el departamento ($montoCorrecto)");
        }
...
function obtenerMarcacionesNocturnasParaExcel($codSucursal, $fechaDesde, $fechaHasta, $codOperario = null) {
    ...
    try {
        $sql = "
            ...
            JOIN sucursales s ON m.sucursal_codigo = s.codigo
            JOIN departamentos d ON s.cod_departamento = d.codigo
            JOIN viatico_reglas_sucursal r ON m.sucursal_codigo = r.sucursal_codigo AND r.tipo = 'Nocturno' AND r.activa = 1
            LEFT JOIN viaticos v ON m.CodOperario = v.cod_operario 
                AND m.fecha = v.fecha 
                AND v.tipo = 'Nocturno'
                AND m.sucursal_codigo = v.sucursal_codigo
            ...
            AND (m.hora_ingreso IS NOT NULL OR m.hora_salida IS NOT NULL)
            -- Excluir operarios asignados principalmente a sucursales que no tienen reglas nocturnas activas
            AND m.CodOperario NOT IN (
                SELECT DISTINCT anc_ex.CodOperario
                FROM AsignacionNivelesCargos anc_ex
                WHERE anc_ex.Sucursal NOT IN (
                    SELECT DISTINCT sucursal_codigo FROM viatico_reglas_sucursal WHERE tipo = 'Nocturno' AND activa = 1
                )
                AND (anc_ex.Fin IS NULL OR anc_ex.Fin >= ?)
            )
        ";
        ...
        // Filtrar por reglas específicas de la regla (días permitidos)
        $resultadosFiltrados = [];
        foreach ($resultados as $viatico) {
            $diasSemanaRegla = $viatico['dias_semana_regla'];
            if (!empty($diasSemanaRegla)) {
                $diaSemana = (int) date('N', strtotime($viatico['fecha']));
                $diasPermitidos = array_map('intval', explode(',', $diasSemanaRegla));
                if (in_array($diaSemana, $diasPermitidos)) {
                    $resultadosFiltrados[] = $viatico;
                }
...
function obtenerViaticosNocturnosBDParaExport($codSucursal, $fechaDesde, $fechaHasta, $codOperario = null) {
    ...
    try {
        $sql = "
            ...
            AND v.fecha BETWEEN ? AND ?
            -- Obtener dinámicamente las sucursales con viáticos configurados y activos
            AND s.codigo IN (SELECT DISTINCT sucursal_codigo FROM viatico_reglas_sucursal WHERE activa = 1)
        ";
        ...
        if (!empty($codSucursal)) {
            ...
            $sql .= " AND v.sucursal_codigo IN (SELECT DISTINCT sucursal_codigo FROM viatico_reglas_sucursal WHERE activa = 1)";
        }
