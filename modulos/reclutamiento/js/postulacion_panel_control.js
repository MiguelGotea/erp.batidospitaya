...
function renderizarTablaSucursales(datos) {
    ...
    datos.forEach(sucursal => {
        ...
        vRow.innerHTML = `
            ...
            <td class="text-center">${generarColumnaDescripcion(sucursal.vendedor_id, 2, sucursal.codigo_sucursal, 'Sucursales', 'Vendedor')}</td>
            <td class="text-center">${generarColumnaHabilidades(sucursal.vendedor_id, 2, sucursal.codigo_sucursal, 'Sucursales', 'Vendedor')}</td>
        `;
        ...
        lRow.innerHTML = `
            ...
            <td class="text-center">${generarColumnaDescripcion(sucursal.lider_id, 5, sucursal.codigo_sucursal, 'Sucursales', 'Líder')}</td>
            <td class="text-center">${generarColumnaHabilidades(sucursal.lider_id, 5, sucursal.codigo_sucursal, 'Sucursales', 'Líder')}</td>
        `;
...
function renderizarTablaAdministrativo(datos) {
    ...
    activos.forEach(cargo => {
        ...
        row.innerHTML = `
            ...
            <td class="text-center">${generarColumnaDescripcion(cargo.config_id, cargo.cod_cargo, 18, 'Administrativo', cargo.nombre_cargo)}</td>
            <td class="text-center">${generarColumnaHabilidades(cargo.config_id, cargo.cod_cargo, 18, 'Administrativo', cargo.nombre_cargo)}</td>
            <td class="text-center">
...
function renderizarTablaProduccion(datos) {
    ...
    activos.forEach(cargo => {
        ...
        row.innerHTML = `
            ...
            <td class="text-center">${generarColumnaDescripcion(cargo.config_id, cargo.cod_cargo, sucursalId, 'Produccion', cargo.nombre_cargo)}</td>
            <td class="text-center">${generarColumnaHabilidades(cargo.config_id, cargo.cod_cargo, sucursalId, 'Produccion', cargo.nombre_cargo)}</td>
            <td class="text-center">
                <div class="form-check form-switch d-flex justify-content-center mb-0">
                    <input class="form-check-input operativo-toggle" type="checkbox" role="switch" checked
                           title="Desactivar cargo" ${!puedeEditar ? 'disabled' : ''}
...
// ========================================
// COLUMNAS DESCRIPCIÓN Y HABILIDADES
// ========================================

function generarColumnaDescripcion(idConfig, cargo, sucursal, area, nombreCargo) {
    const nombre = encodeURIComponent(nombreCargo || '');
    if (!puedeEditar) {
        // Solo lectura: mostrar ícono sin acción si no tiene permiso
        return `<button type="button" class="btn btn-sm btn-outline-secondary btn-col-icono"
                    onclick="abrirModalDescripcion(${idConfig}, ${cargo}, '${sucursal}', '${area}', decodeURIComponent('${nombre}'))"
                    title="Ver descripción del cargo">
                    <i class="bi bi-file-text"></i>
                </button>`;
    }
    return `<button type="button" class="btn btn-sm btn-outline-primary btn-col-icono"
                onclick="abrirModalDescripcion(${idConfig}, ${cargo}, '${sucursal}', '${area}', decodeURIComponent('${nombre}'))"
...
function generarColumnaHabilidades(idConfig, cargo, sucursal, area, nombreCargo) {
    const nombre = encodeURIComponent(nombreCargo || '');
    if (!puedeEditar) {
        return `<button type="button" class="btn btn-sm btn-outline-secondary btn-col-icono"
                    onclick="abrirModalHabilidades(${idConfig}, ${cargo}, '${sucursal}', '${area}', decodeURIComponent('${nombre}'))"
                    title="Ver habilidades del cargo">
                    <i class="bi bi-tags"></i>
                </button>`;
    }
    return `<button type="button" class="btn btn-sm btn-outline-success btn-col-icono"
                onclick="abrirModalHabilidades(${idConfig}, ${cargo}, '${sucursal}', '${area}', decodeURIComponent('${nombre}'))"
...
async function abrirModalDescripcion(idConfig, cargo, sucursal, area, nombreCargo) {
    _descMeta = { idConfig, cargo, sucursal, area };

    document.getElementById('modalDescripcionTitulo').textContent = nombreCargo || 'Cargo';
    document.getElementById('modalDescripcionLoading').style.display = '';
    document.getElementById('modalDescripcionForm').style.display = 'none';
    document.getElementById('txtDescripcion').value = '';
    document.getElementById('txtResponsabilidades').value = '';
    ...
    try {
        const response = await fetch(`ajax/get_detalle_plaza.php?id_config=${idConfig || 0}`);
        const data = await response.json();
        ...
        // Convertir separador | a saltos de línea para mostrar
        document.getElementById('txtResponsabilidades').value = data.responsabilidades
            ? data.responsabilidades.split('|').map(s => s.trim()).join('\n')
            : '';
        document.getElementById('txtRequisitos').value = data.requisitos
            ? data.requisitos.split('|').map(s => s.trim()).join('\n')
            : '';

        document.getElementById('modalDescripcionLoading').style.display = 'none';
        document.getElementById('modalDescripcionForm').style.display = '';
    } catch (error) {
        document.getElementById('modalDescripcionLoading').style.display = 'none';
        document.getElementById('modalDescripcionForm').style.display = '';
...
async function guardarDescripcion() {
    ...
    try {
        // Convertir saltos de línea de vuelta a | para guardar
        const responsabilidades = document.getElementById('txtResponsabilidades').value
            .split('\n').map(s => s.trim()).filter(s => s.length > 0).join('|');
        const requisitos = document.getElementById('txtRequisitos').value
            .split('\n').map(s => s.trim()).filter(s => s.length > 0).join('|');

        const response = await fetch('ajax/guardar_detalle_plaza.php', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                id_config:         _descMeta.idConfig || 0,
                cargo:             _descMeta.cargo,
                sucursal:          _descMeta.sucursal,
                area:              _descMeta.area,
        ...
        Toast.fire({ icon: 'success', title: 'Descripción guardada' });
...
async function abrirModalHabilidades(idConfig, cargo, sucursal, area, nombreCargo) {
    _habMeta = { idConfig, cargo, sucursal, area };

    document.getElementById('modalHabilidadesTitulo').textContent = nombreCargo || 'Cargo';
    document.getElementById('modalHabilidadesLoading').style.display = '';
    document.getElementById('modalHabilidadesForm').style.display = 'none';
    document.getElementById('filtroHabilidades').value = '';
    ...
    try {
        const response = await fetch(`ajax/get_habilidades_catalogo.php?id_config=${idConfig || 0}`);
        const data = await response.json();

        if (!data.success) throw new Error(data.message || 'Error al cargar');

        renderizarHabilidades(data.catalogo, data.seleccionadas);

        document.getElementById('modalHabilidadesLoading').style.display = 'none';
        document.getElementById('modalHabilidadesForm').style.display = '';
        actualizarContadorHabilidades();
    } catch (error) {
        document.getElementById('modalHabilidadesLoading').style.display = 'none';
        document.getElementById('modalHabilidadesForm').style.display = '';
        document.getElementById('listaHabilidades').innerHTML = `<div class="alert alert-danger">Error al cargar habilidades: ${error.message}</div>`;
        console.error('Error al cargar habilidades:', error);
    }
}

function renderizarHabilidades(catalogo, seleccionadas) {
    const lista = document.getElementById('listaHabilidades');
    lista.innerHTML = '';

    // Agrupar por categoría
    const grupos = {};
    catalogo.forEach(hab => {
        const cat = hab.categoria || 'Sin categoría';
    ...
    Object.keys(grupos).sort().forEach(cat => {
        ...
        header.className = 'habilidad-categoria-header';
        ...
        grupos[cat].forEach(hab => {
            const checked = seleccionadas.includes(parseInt(hab.id));
            const item = document.createElement('label');
            item.className = 'habilidad-item';
            item.dataset.nombre = hab.nombre.toLowerCase();
            item.innerHTML = `
                <input type="checkbox" class="form-check-input habilidad-check" value="${hab.id}"
                    ${checked ? 'checked' : ''}
                    ${!puedeEditar ? 'disabled' : ''}
                    onchange="actualizarContadorHabilidades()">
                <span class="small">${hab.nombre}</span>`;
...
function filtrarHabilidades(query) {
    const q = query.toLowerCase().trim();
    document.querySelectorAll('#listaHabilidades .habilidad-item').forEach(item => {
        const nombre = item.dataset.nombre || '';
        item.classList.toggle('habilidad-oculta', q.length > 0 && !nombre.includes(q));
    });
}

function actualizarContadorHabilidades() {
    const total = document.querySelectorAll('#listaHabilidades .habilidad-check:checked').length;
    const cont = document.getElementById('contadorHabilidades');
    if (cont) cont.textContent = `${total} seleccionada${total !== 1 ? 's' : ''}`;
}
...
async function guardarHabilidades() {
    ...
    try {
        const ids = [...document.querySelectorAll('#listaHabilidades .habilidad-check:checked')]
            .map(cb => parseInt(cb.value));

        const response = await fetch('ajax/guardar_habilidades_plaza.php', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                id_config: _habMeta.idConfig || 0,
                cargo:     _habMeta.cargo,
                sucursal:  _habMeta.sucursal,
                area:      _habMeta.area,
                habilidades: ids.join(',')
            })
        });
        const data = await response.json();
        if (!data.success) throw new Error(data.message);
        ...
        const Toast = Swal.mixin({ toast: true, position: 'top-end', showConfirmButton: false, timer: 2500, timerProgressBar: true });
        Toast.fire({ icon: 'success', title: 'Habilidades guardadas' });
