...
<head>
    ...
    <style>
        ...
        .modal-firma-footer button:disabled {
            opacity: .4;
            cursor: not-allowed;
        }
        ...
        .btn-limpiar-firma:hover:not(:disabled) {
            background: #dee2e6;
        }
        ...
        .btn-confirmar-firma:hover:not(:disabled) {
            background: #1f5fa8;
        }
        ...
        .photo-half img.rotate-90 {
            ...
            max-width: 110mm;
            /* El alto del contenedor aprox */
            max-height: 180mm;
            /* El ancho del contenedor aprox */
        }
...
<body onload="/*window.print()*/">
    ...
    <div class="print-options no-print">
        <label class="form-switch">
            <input type="checkbox" id="togglePhotos" <?= $imprimirFotos ? 'checked' : '' ?>
                onchange="cambiarPreferenciaFotos(this.checked)">
            Imprimir Fotos
        </label>
        ...
        <div class="container <?= ($imprimirFotos && !$cabeEnPrimeraHoja && !empty($fotos)) ? 'page-break' : '' ?>">
                ...
                <tbody>
                    <?php foreach ($detalles as $det): ?>
                        <tr>
                            ...
                            <td class="bg-blue"><?= htmlspecialchars($solicitud['ceco_nombre'] ?? $solicitud['ceco']) ?>
                            </td>
                        </tr>
                    <?php endforeach; ?>
            ...
            <table class="footer-table">
                <tr>
                    ...
                    <td style="width: 25%;" class="bg-blue"><?= htmlspecialchars($solicitud['titular'] ?? 'N/A') ?> -
                        <?= htmlspecialchars($solicitud['numero_cuenta'] ?? '') ?></td>
                    <td style="width: 10%;">Banco:</td>
                    <td style="width: 20%;" class="bg-blue"><?= htmlspecialchars($solicitud['banco'] ?? 'N/A') ?>
                        (<?= htmlspecialchars($solicitud['cuenta_moneda'] ?? '') ?>)</td>
                </tr>
            </table>
            ...
            <?php if ($imprimirFotos && $cabeEnPrimeraHoja && !empty($fotos)): ?>
                ...
                <div class="photo-half" style="margin-top: 5mm; border-top: 1px dashed #ccc;">
                    ...
                    <div class="crop-overlay no-print">
                        ...
                        <button class="btn-crop btn-reset-img" id="btn_reset_img_factura_0" style="display:none;"
                            onclick="resetearImagen('img_factura_0')">
                            <i class="fas fa-undo"></i> Original
                        </button>
                    </div>
                    <img src="<?= $fotoActual ?>" id="img_factura_0" data-original="<?= $fotoActual ?>"
                        class="<?= $rotationClass ?>" alt="Factura 1">
                </div>
            <?php endif; ?>
        ...
        <?php if ($imprimirFotos && !empty($fotos)): ?>
            ...
            foreach ($chunks as $index => $chunk):
                ...
                <div class="page-break"
                    style="width: 205mm; margin: 5mm auto 0; border: 1.5px solid #000; height: 265mm; background: #fff;">
                    <?php foreach ($chunk as $subIndex => $fotoPath): ?>
                        ...
                        <div class="photo-half">
                            ...
                            <div class="crop-overlay no-print">
                                ...
                                <button class="btn-crop btn-reset-img" id="btn_reset_<?= $imgId ?>" style="display:none;"
                                    onclick="resetearImagen('<?= $imgId ?>')">
                                    <i class="fas fa-undo"></i> Original
                                </button>
                            </div>
                            <img src="<?= $fotoPath ?>" id="<?= $imgId ?>" data-original="<?= $fotoPath ?>"
                                class="<?= $rotationClass ?>" alt="Factura">
                        </div>
                    <?php endforeach; ?>
                </div>
            <?php endforeach; ?>
            ...
            <div class="modal-firma-box">
                <div class="modal-firma-header">
                    ...
                    <button style="background:none;border:none;color:#fff;font-size:18px;cursor:pointer;"
                        onclick="cerrarModalFirma()" title="Cerrar">&times;</button>
                </div>
                ...
                <div class="modal-firma-footer">
                    ...
                    <button class="btn-confirmar-firma" id="btnConfirmarFirma" onclick="confirmarFirma()" disabled>Confirmar
                        Firma</button>
            ...
            <div class="modal-content border-0 shadow-lg" style="border-radius: 15px;">
                ...
                <div class="modal-body p-0">
                    ...
                    <div class="bg-light p-2 d-flex justify-content-center gap-2 border-top">
                        <button type="button" class="btn btn-sm btn-outline-dark" onclick="cropper.rotate(-90)"
                        ...
                        <button type="button" class="btn btn-sm btn-outline-dark" onclick="cropper.zoom(0.1)"
                            title="Acercar">
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" class="btn btn-pitaya" style="background-color: #51B8AC; color: white;"
