...
if (!$informe_id || $km_inicial === null || $km_inicial === '') {
...
if (isset($_FILES['km_foto_inicial']) && $_FILES['km_foto_inicial']['error'] === UPLOAD_ERR_OK) {
    ...
    $foto_nombre = 'km_ini_' . $informe_id . '_' . time() . '.' . $ext;
...
if (!$foto_nombre && isset($_POST['km_foto_inicial_cam']) && !empty($_POST['km_foto_inicial_cam'])) {
    $data = $_POST['km_foto_inicial_cam'];
    if (preg_match('/^data:image\/(\w+);base64,/', $data, $type)) {
        $data = substr($data, strpos($data, ',') + 1);
        $type = strtolower($type[1]);
        $data = base64_decode($data);
        $foto_nombre = 'km_ini_' . $informe_id . '_' . time() . '.' . $type;
        $target = '../uploads/informes/' . $foto_nombre;
        if (!is_dir('../uploads/informes')) {
            mkdir('../uploads/informes', 0777, true);
        }
...
try {
    ...
    // Actualizar km_inicial (y foto si aplica)
    $ticketModel->actualizarKmInicial($informe_id, floatval($km_inicial), $foto_nombre);
