<?php
/**
 * ajax/formulario_consulta_ia.php
 * Analiza datos de un formulario nuevo (sin ticket_id) con IA y devuelve:
 *   - nivel_urgencia (1-4)
 *   - tiempo_estimado (int, 0 si tercerizado)
 *   - resolucion (texto estructurado)
 *
 * No guarda en BD — el guardado ocurre cuando el formulario es enviado.
 */
...
// Reutilizamos el mismo permiso de consulta_ia
if (!tienePermiso('historial_solicitudes_mantenimiento', 'nuevo_registro', $cargoOperario)) {
    responder(false, 'No tienes permiso para enviar solicitudes.');
}
...
$area_equipo = isset($input['area_equipo']) ? trim($input['area_equipo']) : '';

if (strlen($titulo) < 3 || strlen($descripcion) < 5) {
    responder(false, 'Título o descripción demasiado cortos para analizar.');
}
...
- Área / Equipo afectado: {$area_equipo}
- Tipo de formulario: mantenimiento_general
...
foreach ($proveedores as $prov) {
    try {
        ...
        error_log("formulario_consulta_ia - Fallo proveedor $prov: " . $e->getMessage());
        continue;
    }
}

if (!$respuestaTexto) {
    error_log('formulario_consulta_ia - Todos los proveedores fallaron: ' . implode(' | ', $erroresAcumulados));
...
try {
    ...
    error_log('formulario_consulta_ia - extraerJSON: ' . $e->getMessage() . ' | Raw: ' . substr($respuestaTexto, 0, 500));
...
if ($nivelUrgencia < 1 || $nivelUrgencia > 4) {
    error_log('formulario_consulta_ia - nivel_urgencia fuera de rango: ' . $nivelUrgencia);
    responder(false, 'La IA devolvió un nivel de urgencia inválido. Intenta nuevamente.');
}
...
// ── Respuesta exitosa (sin guardar — el form lo hace al enviarse) ─────────────
...
// ── System Prompt (sincronizado con historial_consulta_ia.php) ──────────────────────
