...
if (!function_exists('obtenerHorariosLiderPorSemanaYSucursal')) {
    function obtenerHorariosLiderPorSemanaYSucursal($numeroSemana, $codSucursal) {
        global $conn;
        
        // Primero obtener el ID de la semana, acá se compuso que en vez de usar el id en la tabla SemanasSistema sea la columna numero_semana que es el valor de semana según sistema
        ...
        ");
        $stmt->execute([$semana['id'], $codSucursal]);
...
if (false) {
    ...
    // Verificar si es líder de sucursal (cargo 5)
    if (verificarAccesoCargo([5])) {
        // Obtener solo las sucursales donde es líder
        $sucursales = obtenerSucursalesLider($usuario['CodOperario']);
    } else {
        // Si no es admin ni líder, redirigir
        header('Location: /index.php');
        exit();
    }
}

// Si no hay sucursales asignadas al líder
if (empty($sucursales)) {
...
// Obtener semana actual y semana anterior
...
if ($semanaActual) {
    $stmt = $conn->prepare("
        ...
        LIMIT 1
    ");
    $stmt->execute([$semanaActual['numero_semana'] - 1]);
    $semanaAnterior = $stmt->fetch();
}
...
$hayRegistrosLider = !empty($horariosLider);
...
if ($sucursalSeleccionada && $semanaSeleccionada && $semana) {
    ...
    } elseif ($hayRegistrosLider) {
        $estadoHorario = 'solo-lider';
    }
}
...
$estados = [
    ...
    'solo-lider' => [
        'clase' => 'status-info',
        'mensaje' => 'Solo hay registro de horario del líder, pendiente de operaciones'
    ],
    'sin-registros' => [
...
<body>
    <div class="container">
        ...
        <div class="filters">
            <div class="filter-group">
                ...
                <select id="semana" name="semana" onchange="cambiarSemana()">
                    <?php if ($semanaActual): ?>
                        <option value="<?= $semanaActual['numero_semana'] ?>" <?= $semanaSeleccionada == $semanaActual['numero_semana'] ? 'selected' : '' ?>>
                            Semana <?= $semanaActual['numero_semana'] ?> (Actual, <?= formatoFecha($semanaActual['fecha_inicio']) ?> al <?= formatoFecha($semanaActual['fecha_fin']) ?>)
                        </option>
                    <?php endif; ?>
                    <?php if ($semanaAnterior): ?>
                        <option value="<?= $semanaAnterior['numero_semana'] ?>" <?= $semanaSeleccionada == $semanaAnterior['numero_semana'] ? 'selected' : '' ?>>
                            Semana <?= $semanaAnterior['numero_semana'] ?> (Anterior, <?= formatoFecha($semanaAnterior['fecha_inicio']) ?> al <?= formatoFecha($semanaAnterior['fecha_fin']) ?>)
                        </option>
                    <?php endif; ?>
                </select>
            </div>
        ...
        <?php if ($semanaSeleccionada && $sucursalSeleccionada && $semana): ?>
            <?php if (!$hayRegistrosLider && !$hayRegistrosOperaciones): ?>
                    ...
                    <table>
                        <thead>
                            <tr>
                                ...
                                ?>
                                <th style="display:none;" rowspan="2">Total Horas</th>
                            </tr>
                            <tr>
                                <?php 
                                $fechaActual = new DateTime($semana['fecha_inicio']);
                        ...
                        <tbody>
                            ...
                            ?>
                                <tr data-operario="<?= $operario['CodOperario'] ?>"
                                    ...
                                    <td style="display:none;" class="total-hours">
