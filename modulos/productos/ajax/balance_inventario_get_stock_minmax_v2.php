<?php
/* ============================================================
   AJAX: Stock Mín y Stock Máx Final para un producto — v2
   modulos/productos/ajax/balance_inventario_get_stock_minmax_v2.php
 ...
 */
function calcularCicloRealDias_SMM(?array $planCat): ?float {
    if (!$planCat || !($planCat['activo'] ?? 0)) return null;
    if ($planCat['tipo_frecuencia'] === 'n_semanas') {
        return (int)($planCat['intervalo_semanas'] ?? 1) * 7.0;
    }
 ...
 */
function calcularDiasPreparacion_SMM(?array $planCat): ?float {
    if (!$planCat || !($planCat['activo'] ?? 0)) return null;
...
try {
    ...
    // Plan de despacho de la sucursal para la categoría del producto
    $planCatSMM = null;
    if ($cat && $codSuc) {
        $stmtPDSMM = $conn->prepare("
            ...
            LIMIT 1
        ");
        $stmtPDSMM->execute([$codSuc, $cat]);
        $planCatSMM = $stmtPDSMM->fetch(PDO::FETCH_ASSOC) ?: null;
    }

    $cicloRealSMM = calcularCicloRealDias_SMM($planCatSMM);
    $diasPrepSMM  = calcularDiasPreparacion_SMM($planCatSMM);
