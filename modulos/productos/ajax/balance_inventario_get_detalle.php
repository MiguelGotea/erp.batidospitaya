...
try {
    ...
    if (!$prodMeta) { echo json_encode(['ok' => false, 'msg' => 'Producto no encontrado']); exit(); }
    ...
    if (empty($allCods)) { echo json_encode(['ok'=>true, 'registros'=>[], 'producto'=>$prodMeta]); exit(); }
    ...
    if (!empty($ingsRel) || !empty($allCods)) {
        ...
        if (!empty($ingsRel)) { $whereEx .= " OR sr.CodIngrediente IN (".implode(',', array_fill(0, count($ingsRel), '?')).")"; $pVal = array_merge($pVal, $ingsRel); }
        if (!empty($allCods)) { $whereEx .= " OR sr.codporcion IN ($phCods)"; $pVal = array_merge($pVal, $allCods); }
        ...
        foreach ($stmtV->fetchAll(PDO::FETCH_ASSOC) as $f) {
            $target = null; $isP1 = false;
            $cp = $f['codporcion'] ? (int)$f['codporcion'] : null;
            $ci = $f['CodIngrediente'];
            
            if ($cp && isset($codMap[$cp])) { $target = $cp; $isP1 = true; }
            elseif ($ci && isset($cotP2P3[$ci])) {
                if (isset($cotP2P3[$ci]['p2']) && isset($codMap[$cotP2P3[$ci]['p2']])) $target = $cotP2P3[$ci]['p2'];
                elseif (isset($cotP2P3[$ci]['p3']) && isset($codMap[$cotP2P3[$ci]['p3']])) $target = $cotP2P3[$ci]['p3'];
            }

            if ($target) {
                $val = $f['total'] * $codMap[$target]['factor'];
                if ($isP1) $val = round($val * 2) / 2;
                $fec = $f['Fecha'];
    ...
    foreach ($registros as $reg) if (isset($totales[$reg['tipo']])) $totales[$reg['tipo']] += $reg['qty_base'];
