...
try {
    ...
    // ── 5. Maestro → Base map ─────────────────────────────────────────────
    $rMB = $conn->prepare("SELECT pp.id, pp.id_unidad_producto AS unid, pp.cantidad AS cant, pp.id_producto_maestro AS mid FROM producto_presentacion pp WHERE pp.presentacion_basica_inventario=1 AND pp.Activo='SI'");
    $rMB->execute();
    $maestroToBase = [];
    ...
    foreach ($idsPP as $targetId) {
        ...
        // en ese caso permitimos fecha_D1 dentro de la semana actual (>= ayer) porque
        // la proyección arranca desde el final del día de ayer (anchorVal) hacia adelante.
