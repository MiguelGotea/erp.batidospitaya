...
try {
    if (empty($id)) {
        // Insertar
        $sql = "INSERT INTO talento_valores (icono, titulo, descripcion, orden, activo, usuario_creador, fecha_creacion)
                VALUES (?, ?, ?, ?, ?, ?, NOW())";
        $stmt = $conn->prepare($sql);
        $stmt->execute([$icono, $titulo, $descripcion, $orden, $activo, $codOperario]);
        echo json_encode(['success' => true, 'mensaje' => 'Valor corporativo creado con éxito']);
    } else {
        // Actualizar
        $sql = "UPDATE talento_valores 
                SET icono = ?, titulo = ?, descripcion = ?, orden = ?, activo = ?, usuario_modifica = ?, fecha_modificacion = NOW()
                WHERE id = ?";
        $stmt = $conn->prepare($sql);
        $stmt->execute([$icono, $titulo, $descripcion, $orden, $activo, $codOperario, $id]);
        echo json_encode(['success' => true, 'mensaje' => 'Valor corporativo actualizado con éxito']);
    }
} catch (Exception $e) {
