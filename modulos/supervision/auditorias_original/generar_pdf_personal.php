...
require '../../../core/database/conexion.php'; // Cambiado: anteriormente llamaba al conexion de auditor�as, ahora llama al del core;
...
try {
    ...
    $stmt = $conn->prepare("SELECT ap.*, ap.tipo_auditoria, ap.promedio_personal, ap.comentarios,
    ...
    // Obtener las fotos asociadas de la tabla auditoria_personal_fotos
    $stmtFotos = $conn->prepare("SELECT ruta_foto FROM auditoria_personal_fotos WHERE auditoria_id = :id");
    ...
    $pdf->Cell(0, 10, prepararTexto('AUDITORIA DE PERSONAL'), 0, 1, 'C');
    $pdf->Ln(5);
    ...
    $pdf->SetFont('Arial', 'B', 12);
    $pdf->Cell(40, 8, prepararTexto('Verificador:'));
    $pdf->SetFont('Arial', '', 12);
    ...
    $pdf->SetTextColor($colorSecundario[0], $colorSecundario[1], $colorSecundario[2]);
    $pdf->Cell(0, 10, prepararTexto('PRESENTACION DEL PERSONAL'), 0, 1, 'C');

    // Promedio
    $pdf->SetFillColor($colorPrincipal[0], $colorPrincipal[1], $colorPrincipal[2]);
    $pdf->SetTextColor(255, 255, 255);
    $pdf->SetFont('Arial', 'B', 12);
    $pdf->Cell(0, 10, prepararTexto('PROMEDIO: ') . (!empty($registro['promedio_personal']) ? $registro['promedio_personal'] : 'N/A'), 0, 1, 'C', true);
    $pdf->Ln(8);
    ...
    $items = [
        'Uñas cortas y limpias' => $registro['presentacion_personal_2_1'] ?? null,
        'Uso de malla para cabello' => $registro['presentacion_personal_2_2'] ?? null,
        'Varones bien rasurados' => $registro['presentacion_personal_2_3'] ?? null,
        'Maquillaje sutil (mujeres)' => $registro['presentacion_personal_2_4'] ?? null,
        'Sin anillos/pulseras' => $registro['presentacion_personal_2_5'] ?? null,
        'Condiciones higiénicas' => $registro['presentacion_personal_2_6'] ?? null,
        'Pin de identificación' => $registro['presentacion_personal_2_7'] ?? null,
        'Porte y Aspecto' => $registro['presentacion_personal_2_8'] ?? null
    ];
