...
function calcularPorcentajeReclamos($cantidad)
{
    ...
    if ($cantidad == 2) return 80;
    if ($cantidad == 3) return 60;
...
{
    if ($cantidad <= 0 || $meta_acumulada <= 0) return 0;
...
{
    if ($pct_real <= 0 || $meta <= 0) return 0;
    if ($pct_real >= $meta) return 100;
...
try {
    ...
    foreach ($sucursales as $cod => $nombre) {
        ...
        if ($limpieza['cantidad'] > 0) $arr_limpieza[] = (float)$limpieza['promedio'];
        if ($personal['cantidad'] > 0) $arr_personal[] = (float)$personal['promedio'];
        if ($servicio['cantidad'] > 0) $arr_servicio[] = (float)$servicio['promedio'];
