...
// Obtener ID del aviso a editar
$avisoId = isset($_GET['id']) ? (int)$_GET['id'] : 0;

// Si no hay ID válido, redirigir
if ($avisoId <= 0) {
    $_SESSION['error_message'] = "ID de aviso no válido";
    header("Location: index_avisos.php");
    exit();
}

// Cargar datos del aviso existente
$aviso = null;
$sucursalesSeleccionadas = [];
$archivosAdjuntos = [];

try {
    // Obtener información básica del aviso
    $stmt = $conn->prepare("
        SELECT a.*, CONCAT(o.Nombre, ' ', o.Apellido) as author 
    ...
    if (!$aviso) {
        $_SESSION['error_message'] = "Aviso no encontrado";
        header("Location: index_avisos.php");
        exit();
    }
    
    // Obtener sucursales seleccionadas para este aviso
    $stmt = $conn->prepare("
        SELECT branch_id FROM announcement_branches WHERE announcement_id = ?
    ");
    $stmt->execute([$avisoId]);
    $sucursalesSeleccionadas = $stmt->fetchAll(PDO::FETCH_COLUMN);
    ...
    ");
    $stmt->execute([$avisoId]);
    $archivosAdjuntos = $stmt->fetchAll(PDO::FETCH_ASSOC);
    
} catch (PDOException $e) {
    error_log("Error al cargar aviso: " . $e->getMessage());
    $_SESSION['error_message'] = "Error al cargar el aviso";
    header("Location: index_avisos.php");
    exit();
}
    ...
    if (empty($errors)) {
        try {
            ...
            $stmt = $conn->prepare("
                ...
                WHERE id = ?
            ");
            $stmt->execute([$title, $content, $avisoId]);
            
            // Eliminar relaciones de sucursales existentes
            $stmt = $conn->prepare("DELETE FROM announcement_branches WHERE announcement_id = ?");
            $stmt->execute([$avisoId]);
            
            // Insertar nuevas relaciones de sucursales
            $stmt = $conn->prepare("
                INSERT INTO announcement_branches (announcement_id, branch_id) 
                VALUES (?, ?)
            ");
            foreach ($selectedBranches as $branchCodigo) {
                $stmt->execute([$avisoId, $branchCodigo]);
            }
            ...
            if (!empty($filesToDelete)) {
                ...
                $stmt = $conn->prepare("
                    ...
                    WHERE id IN ($placeholders) AND announcement_id = ?
                ");
                $params = array_merge($filesToDelete, [$avisoId]);
                $stmt->execute($params);
                
                $filesToRemove = $stmt->fetchAll(PDO::FETCH_COLUMN);
            ...
            if (!empty($_FILES['documents']['name'][0]) || !empty($_FILES['media']['name'][0])) {
                ...
                if (!empty($_FILES['documents']['name'][0])) {
                    processUploadedFiles($_FILES['documents'], $avisoId, $conn, $uploadDir);
                }
                
                // Procesar multimedia
                if (!empty($_FILES['media']['name'][0])) {
                    processUploadedFiles($_FILES['media'], $avisoId, $conn, $uploadDir);
                }
            }
            ...
            $_SESSION['success_message'] = "Aviso actualizado exitosamente";
            header("Location: index_avisos.php");
            exit();
            ...
            $errors[] = "Ocurrió un error al actualizar el aviso. Por favor intente nuevamente.";
        }
    }
}
?>
...
<head>
    ...
    <title>Editar Aviso - Batidos Pitaya</title>
    ...
    <style>
        * {
        ...
        .existing-file input[type="checkbox"] {
            margin-right: 8px;
        }
...
<body>
    ...
    <div class="header-container">
        ...
        <div class="buttons-container">
            <a href="index_avisos.php" class="btn-agregar <?php echo $es_pagina_avisos ? 'activo' : ''; ?>">
                <i class="fas fa-bullhorn"></i> <span class="btn-text">Anuncios</span>
            </a>
            <a style="display:none;" href="index.php" class="btn-agregar <?php echo $es_pagina_auditorias ? 'activo' : ''; ?>">
                <i class="fas fa-clipboard-check"></i> <span class="btn-text">Auditorías</span>
            </a>
            <a style="display:none;" href="index_auditorias_publico.php" class="btn-agregar" target="_blank">
                <i class="fas fa-users"></i> <span class="btn-text">Público</span>
            </a>
            <a style="display:none;" href="logout.php" class="btn-agregar">
                <i class="fas fa-sign-out-alt"></i> <span class="btn-text">Cerrar Sesión</span>
            </a>
            <a href="index_avisos.php" class="btn-agregar">
                <i class="fas fa-folder-open"></i> <span class="btn-text">Regresar</span>
            </a>
        </div>
    </div>
    
    <div class="contenedor-principal">
        <h2 style="color: #0E544C; margin-bottom: 15px;">EDITAR AVISO</h2>
        ...
        <div class="form-container">
            <form id="avisoForm" method="POST" enctype="multipart/form-data">
                <!-- Campo oculto para el ID del aviso -->
                ...
                <div class="current-time">
                    ...
                    Creado el: <?= formatFechaEspanolAviso($aviso['created_at']) ?> 
                    <?php if ($aviso['author']): ?>
                        por: <?= htmlspecialchars($aviso['author']) ?>
                    <?php endif; ?>
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="title" name="title" required 
                           ...
                           value="<?= htmlspecialchars($_POST['title'] ?? $aviso['title']) ?>">
                </div>
                
                <!-- Contenido -->
                <div class="form-group">
                    <label for="content">Contenido:</label>
                    <textarea style="text-align:left !important;" id="content" name="content" required><?= 
                        htmlspecialchars($_POST['content'] ?? $aviso['content']) 
                    ?></textarea>
                </div>
                ...
                <div class="form-group">
                    ...
                    <div class="branches-select">
                        ...
                        <div class="branch-checkbox" style="grid-column: 1 / -1;">
                            ...
                            <label for="select_all_branches" style="font-weight: bold;">TODAS LAS SUCURSALES</label>
                        </div>
                        
                        <?php foreach ($sucursales as $sucursal): 
                            $isChecked = in_array($sucursal['codigo'], $sucursalesSeleccionadas);
                            if (isset($_POST['branches'])) {
                                $isChecked = in_array($sucursal['codigo'], $_POST['branches']);
                            }
                        ?>
                            <div class="branch-checkbox">
                                <input type="checkbox" id="branch_<?= $sucursal['codigo'] ?>" 
                                       name="branches[]" value="<?= $sucursal['codigo'] ?>"
                                       class="branch-checkbox-item"
                                       <?= $isChecked ? 'checked' : '' ?>>
                                <label for="branch_<?= $sucursal['codigo'] ?>"><?= htmlspecialchars($sucursal['nombre']) ?></label>
                            </div>
                        <?php endforeach; ?>
                    </div>
                </div>
                ...
                <?php if (!empty($archivosAdjuntos)): ?>
                    <div class="file-upload-section">
                        <div class="file-upload-title">
                            <i class="fas fa-paperclip"></i> Archivos adjuntos existentes
                        </div>
                        <div class="existing-files">
                            ...
                            foreach ($archivosAdjuntos as $archivo): 
                                $isDocument = in_array($archivo['file_type'], $documentTypes);
                                $isMultimedia = in_array($archivo['file_type'], $multimediaTypes);
                            ?>
                                <div class="existing-file">
                                    <input type="checkbox" id="delete_<?= $archivo['id'] ?>" 
                                           name="delete_files[]" value="<?= $archivo['id'] ?>">
                                    <label for="delete_<?= $archivo['id'] ?>">
                                        <i class="fas fa-trash-alt" style="color: #ff6b6b;"></i>
                                        <?= htmlspecialchars($archivo['file_name']) ?>
                                    </label>
                                </div>
                            <?php endforeach; ?>
                        </div>
                ...
                <div class="file-upload-section">
                    <div class="file-upload-title">
                        <i class="fas fa-file-alt"></i> Agregar nuevos documentos (PDF, Word, Excel, PowerPoint)
                    </div>
                ...
                <div class="file-upload-section">
                    <div class="file-upload-title">
                        <i class="fas fa-images"></i> Agregar nuevas imágenes/videos (Instagram Style)
                    </div>
                    <div class="file-upload-box">
                        <label class="file-upload-label">
                    ...
                    <!-- Galería estilo Instagram para previsualización -->
                    ...
                    <?php if (!empty($archivosAdjuntos)): 
                        $multimediaFiles = array_filter($archivosAdjuntos, function($a) use ($multimediaTypes) {
                            return in_array($a['file_type'], $multimediaTypes);
                        });
                    ?>
                        <?php if (!empty($multimediaFiles)): ?>
                            <div class="file-upload-title" style="margin-top: 20px;">
                                <i class="fas fa-images"></i> Multimedia existente
                            </div>
                            <div class="instagram-gallery">
                                <?php foreach ($multimediaFiles as $archivo): 
                                    $isImage = strpos($archivo['file_type'], 'image') !== false;
                                    $isVideo = strpos($archivo['file_type'], 'video') !== false;
                                ?>
                                    <div class="gallery-item">
                                        <?php if ($isImage): ?>
                                            <img src="<?= htmlspecialchars($archivo['file_path']) ?>" 
                                                 alt="<?= htmlspecialchars($archivo['file_name']) ?>">
                                        <?php elseif ($isVideo): ?>
                                            <video controls>
                                                <source src="<?= htmlspecialchars($archivo['file_path']) ?>" 
                                                        type="<?= htmlspecialchars($archivo['file_type']) ?>">
                                            </video>
                                            <i class="fas fa-play play-icon"></i>
                                        <?php endif; ?>
                                        <div class="remove-media" title="Marcar para eliminar">
                                            <input type="checkbox" id="delete_<?= $archivo['id'] ?>" 
                                                   name="delete_files[]" value="<?= $archivo['id'] ?>" 
                                                   style="display: none;">
                                            ×
                ...
                <div class="form-actions">
                    <button type="button" class="btn btn-secondary" onclick="window.location.href='index_avisos.php'">
                        <i class="fas fa-times"></i> Cancelar
                    </button>
                    <button type="submit" class="btn btn-primary" onclick="return confirmSubmit()">
                        <i class="fas fa-save"></i> Actualizar Aviso
                    </button>
                </div>
            </form>
        </div>
    ...
    <script>
        ...
        function validateAndSubmit(event) {
            ...
            if (confirm('¿Está seguro que desea actualizar este aviso?')) {
        ...
        // Manejar la eliminación de archivos existentes en la galería
        function setupExistingMediaDeletion() {
            document.querySelectorAll('.gallery-item .remove-media').forEach(btn => {
                btn.addEventListener('click', function() {
                    const checkbox = this.querySelector('input[type="checkbox"]');
                    if (checkbox) {
                        checkbox.checked = !checkbox.checked;
                        this.style.backgroundColor = checkbox.checked ? 'rgba(0,255,0,0.7)' : 'rgba(255,0,0,0.7)';
                    }
                });
            });
        }
        ...
        // O también puedes hacerlo al perder el foco del campo
