...
$(document).ready(function () {
    ...
    $(window).on('scroll', function (e) {
        // Solo cerrar si el scroll es significativo Y no hay un input de filtro enfocado
        const esInputEnfocado = $('.filter-search, #mesCalendario, #añoCalendario').is(':focus');
        if (panelFiltroAbierto && !esInputEnfocado && Math.abs($(window).scrollTop() - scrollTopInicial) > 100) {
            cerrarTodosFiltros();
        }
    });
    ...
    $(window).on('resize', function () {
        ...
        // Solo cerrar si el ancho cambia (la aparición del teclado en móvil suele cambiar solo el alto)
...
// L�gica de Terminaci�n de Contrato (Tra�da y adaptada de editar_colaborador.php)
...
// Manejar env�o del formulario de terminaci�n
.ready(function() {
