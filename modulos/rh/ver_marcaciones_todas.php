...
// El auth.php ya maneja la autenticación básica
...
$marcaciones = obtenerMarcacionesConHorariosProgramados($sucursalSeleccionada, $fechaDesde, $fechaHasta, $filtroActivo,
$busqueda, $operario_id, 'sucursal');
} elseif ($modoVista === 'todas') {
$marcaciones = obtenerMarcacionesConHorariosProgramados(null, $fechaDesde, $fechaHasta, $filtroActivo, $busqueda,
$operario_id, 'todas');
}
...
if ($salida < $entrada) { $salida->add(new DateInterval('P1D'));
    }
    ...
    */
    function obtenerMarcacionesConHorariosProgramados($codSucursal, $fechaDesde, $fechaHasta, $filtroActivo = 'todos',
    $busqueda = '', $operario_id = 0, $modoVista = 'sucursal')
    {
    global $conn;
    ...
    WHERE ss.fecha_inicio <= ? AND ss.fecha_fin>= ?
        ...
        for ($fecha = clone $fechaInicio; $fecha <= $fechaFin; $fecha->modify('+1 day')) {
            ...
            if ($fechaStr < $fechaDesde || $fechaStr> $fechaHasta || $fechaStr > $fechaHoy) {
                ...
                $faltaYaRegistrada = verificarFaltaYaRegistrada($horario['cod_operario'], $horario['cod_sucursal'],
                $fechaStr);
                ...
                if (isset($_GET['exportar_excel']) || isset($_GET['exportar_faltas']) ||
                ...
                echo '<table border="1">';
                    ...
                    if ($salidaProg < $entradaProg) { $salidaProg->add(new DateInterval('P1D'));
                        }
                        ...
                        if ($salida < $entrada) { $salida->add(new DateInterval('P1D'));
                            ...
                            echo '<tr>';
                                ...
                                echo '<td>' . number_format($totalOperario, 2) . ' hrs</td>'; // USA EL TOTAL
                                PRECALCULADO
                                echo '</tr>';
                            }
                ...
                $reporteFaltas = generarReporteFaltas($modoVista, $sucursalSeleccionada, $fechaDesde, $fechaHasta,
                $filtroActivo, $operario_id);
                ...
                $reporteTardanzas = generarReporteTardanzas($modoVista, $sucursalSeleccionada, $fechaDesde, $fechaHasta,
                $filtroActivo, $operario_id);
                ...
                return $horaNum >= 20 && $horaNum < 24; // 8:00 PM a 11:59 PM } // Función para determinar el tipo de
                    horario según la hora function obtenerTipoHorario($hora) { if (empty($hora)) return 'normal' ;
                    $horaObj=DateTime::createFromFormat('H:i:s', $hora); if (!$horaObj) return 'normal' ; $horaNum=(int)
                    $horaObj->format('H');

                    if ($horaNum >= 20 && $horaNum < 24) { // 8:00 PM a 11:59 PM return 'nocturno' ; } elseif ($horaNum>
                        = 12 && $horaNum < 20) { // 12:00 PM a 7:59 PM return 'tarde' ; } else { // Antes de las 12:00
                            PM return 'normal' ; } } /** * Genera reporte de faltas - VERSIÓN CORREGIDA */ function
                            generarReporteFaltas($modoVista, $codSucursal, $fechaDesde, $fechaHasta, $filtroActivo,
                            $operario_id) { global $conn; try { // Obtenemos todos los operarios con su última
                            asignación de sucursal $sqlOperarios="SELECT 
            o.CodOperario,
            o.Operativo,
            CONCAT(
                IFNULL(o.Nombre, ''), ' ', 
        ...
        ))" ; $params=[]; // Aplicar filtros según modo de vista if ($modoVista==='sucursal' && $codSucursal &&
                            $codSucursal !=='todas' ) { $sqlOperarios .=" AND (COALESCE(
                (SELECT anc.Sucursal 
                 FROM AsignacionNivelesCargos anc 
                 WHERE anc.CodOperario = o.CodOperario 
                 AND (anc.Fin IS NULL OR anc.Fin >= CURDATE())
                 ...
                 LIMIT 1),
                o.Sucursal
            ) = ?)" ; $params[]=$codSucursal; } // Aplicar filtro de operario específico if ($operario_id> 0) {
                            ...
                            WHERE DATE(?) + INTERVAL (a.a + (10 * b.a)) DAY <= ? ) h WHERE h.fecha BETWEEN ? AND ? AND
                                ...
                                NULL OR m.hora_salida IS NOT NULL) )"; $stmtFaltas=$conn->
                                ...
                                * Genera reporte de tardanzas - VERSIÓN MODIFICADA para combinar código de contrato y
                                nombre
                                */
                                function generarReporteTardanzas($modoVista, $codSucursal, $fechaDesde, $fechaHasta,
                                $filtroActivo, $operario_id)
                                {
                                global $conn;
                                ...
                                // TARDANZAS JUSTIFICADAS: Tardanzas manuales con estado "Justificado" para este
                                contrato
                                ...
                                * Verifica si ya existe una tardanza manual registrada para un operario en una fecha
                                específica
                                */
                                function verificarTardanzaYaRegistrada($codOperario, $codSucursal, $fecha, $codContrato
                                = null)
                                {
                                global $conn;
                                ...
                                <head>
                                    ...
                                    <link rel="stylesheet"
                                        href="/core/assets/css/global_tools.css?v=<?php echo mt_rand(1, 10000); ?>">
                                    <link rel="stylesheet"
                                        href="css/ver_marcaciones_todas.css?v=<?php echo mt_rand(1, 10000); ?>">
                                </head>
                                ...
                                <body>
                                    ...
                                    <div class="main-container">
                                        <div class="sub-container">
                                            <?php echo renderHeader($usuario, false, 'Visualización de Marcaciones'); ?>
                                                ...
                                                <div class="filters">
                                                    ...
                                                    <?php if ($esLider): ?>
                                                        <div class="filter-group">
                                                            ...
                                                            <select id="numero_semana" name="numero_semana"
                                                                onchange="seleccionarSemana(this.value)"
                                                                style="width: 100%;">
                                                            ...
                                                            <small
                                                                style="color: #0E544C; font-style: italic; display:none;">
                                                    ...
                                                    <div class="filter-group">
                                                        ...
                                                        <?php if ($esLider): ?>
                                                            ...
                                                            <select id="operario_id" name="operario_id"
                                                                onchange="aplicarFiltrosLider()" style="width: 100%;">
                                                                <option value="<?= $_SESSION['usuario_id'] ?>"
                                                                    <?= $operario_id == $_SESSION['usuario_id'] ? 'selected' : '' ?>>
                                                                    Mis marcaciones
                                                                </option>
                                                                ...
                                                                <?php foreach ($operariosFiltro as $op):
                                                                    if ($op['CodOperario'] != $_SESSION['usuario_id']): // Excluir al líder mismo
                                                                        ...
                                                                        <option value="<?= $op['CodOperario'] ?>"
                                                                            <?= $operario_id == $op['CodOperario'] ? 'selected' : '' ?>>
                                                        ...
                                                        <?php else: ?>
                                                            ...
                                                            <div style="display: flex; gap: 5px;">
                                                                <input type="text" id="operario" name="operario"
                                                                    placeholder="Escriba para buscar..." value="<?php
                                                                    if ($operario_id > 0) {
                                                                ...
                                                                <?php if ($operario_id > 0): ?>
                                                                    <button type="button" onclick="limpiarFiltroOperario()"
                                                    ...
                                                    <div class="filter-group">
                                                        ...
                                                        <input type="date" id="desde" name="desde"
                                                            value="<?= $fechaDesde ?>" max="<?= date('Y-m-d') ?>">
                                                    </div>

                                                    <div class="filter-group">
                                                        <label for="hasta">Hasta</label>
                                                        <input type="date" id="hasta" name="hasta"
                                                            value="<?= $fechaHasta ?>" max="<?= date('Y-m-d') ?>">
                                                    </div>
                                                        ...
                                                        <table id="tabla-marcaciones">
                                                                ...
                                                                <?php foreach ($marcaciones as $marcacion):
                                                                    ...
                                                                    <tr>
                                                                        ...
                                                                        <td class="text-center" style="display:none;">
                                                                            <span
                                                                                class="status-<?= $marcacion['estado_actual'] ? 'activo' : 'inactivo' ?>">
                                                                        ...
                                                                        <td class="text-center">
                                                                            <?php if ($marcacion['estado_dia'] === 'Activo'): ?>
                                                                                <span
                                                                                    class="status-activo"><?= htmlspecialchars($marcacion['estado_dia']) ?></span>
                                                                            <?php else: ?>
                                                                                <span
                                                                        ...
                                                                        <td style="display:none;" class="text-center">
                                                                            <?php if ($diferenciaEntrada !== null): ?>
                                                                                <?php if ($diferenciaEntrada > 0): ?>
                                                                                    <span
                                                                                        class="diferencia-tarde">+<?= $diferenciaEntrada ?>
                                                                                        min</span>
                                                                                <?php elseif ($diferenciaEntrada < 0): ?>
                                                                                    <span
                                                                                        class="diferencia-temprano"><?= $diferenciaEntrada ?>
                                                                                        min</span>
                                                                                <?php else: ?>
                                                                                    <span>0 min</span>
                                                                                <?php endif; ?>
                                                                        ...
                                                                        <td style="display:none;" class="text-center">
                                                                            <?php if ($diferenciaSalida !== null): ?>
                                                                                <?php if ($diferenciaSalida > 0): ?>
                                                                                    <span
                                                                                        class="diferencia-tarde">+<?= $diferenciaSalida ?>
                                                                                        min</span>
                                                                                <?php elseif ($diferenciaSalida < 0): ?>
                                                                                    <span
                                                                                        class="diferencia-temprano"><?= $diferenciaSalida ?>
                                                                                        min</span>
                                                                                <?php else: ?>
                                                                                    <span>0 min</span>
                                                                                <?php endif; ?>
                                                            ...
                                                            <?= $tardanza ? 'true' : 'false' ?>
                                                                                            ...
                                                                                            <i class="fas fa-clock"
                                                                                                style="color: #ffc107; display:none;"></i>
                                                                                            Justificar Tardanza
                                                                                        </button>
                                                                                    <?php elseif ($tardanzaYaRegistrada): ?>
                                                                                        ...
                                                                                        <span class="text-muted" style="font-size: 0.8em;">
                                                                                            <i class="fas fa-check-circle"
                                                                                                style="color: #9C0000;"></i> Tardanza ya
                                                                                            solicitada
                                                                                        </span>
                                                                                    <?php else: ?>
                                                            ...
                                                            '<?= $marcacion['hora_salida_programada'] ?>'
                                                                                            ...
                                                                                            <i class="fas fa-user-times"
                                                                                                style="color: #dc3545; display:none;"></i>
                                                                                            Justificar Falta
                                                                                        </button>
                                                                                    <?php else: ?>
                                                                                        <span class="text-muted"
                                                                                ...
                                                                                <?php elseif (!$marcacion['tiene_horario']): ?>
                                                                                    <span class="text-muted"
                                                                                        style="font-size: 0.8em; display:none;">Sin
                                                                                        horario</span>

                                                                                    <!-- Opción 5: A tiempo o sin acción -->
                                                                                <?php else: ?>
                                                                                    <span class="text-muted"
                                                                                        style="font-size: 0.8em; display:none;">A
                                                                                        tiempo</span>
                                                                                <?php endif; ?>
                                                                            </td>
                                                                        <?php endif; ?>
                                                ...
                                                <div class="modal-content" style="max-width: 500px;">
                                                    <div class="modal-header">
                                                        ...
                                                        <button class="modal-close"
                                                            onclick="cerrarModalTardanza()">&times;</button>
                                                    </div>
                                                    <form id="formTardanzaManual" method="post"
                                                        enctype="multipart/form-data">
                                                        <input type="hidden" name="registrar_tardanza_marcacion"
                                                            value="1">
                                                        <input type="hidden" id="tm_cod_operario" name="cod_operario">
                                                        <input type="hidden" id="tm_cod_sucursal" name="cod_sucursal">
                                                        <input type="hidden" id="tm_fecha_tardanza"
                                                            name="fecha_tardanza">
                                                        ...
                                                        <div class="modal-body">
                                                            ...
                                                            <div class="form-group">
                                                                <label for="tm_tipo_justificacion"
                                                                    class="form-label">Tipo de Justificación:</label>
                                                                <select id="tm_tipo_justificacion"
                                                                    name="tipo_justificacion" class="form-select"
                                                                    required>
                                                            ...
                                                            <div class="form-group">
                                                                <label for="tm_foto" class="form-label">Foto
                                                                    (obligatorio):</label>
                                                                <input type="file" id="tm_foto" name="foto"
                                                                    class="form-input" accept="image/*" required>
                                                                <img id="tm_foto_preview" class="photo-preview" src="#"
                                                                    alt="Vista previa" style="display: none;">
                                                            </div>

                                                            <div class="form-group">
                                                                <label for="tm_observaciones"
                                                                    class="form-label">Observaciones:</label>
                                                                <textarea id="tm_observaciones" name="observaciones"
                                                                    class="form-textarea"
                                            ...
                                            <div class="modal" id="modalFaltaManual" style="display: none;">
                                                <div class="modal-content" style="max-width: 500px;">
                                                    <div class="modal-header">
                                                        ...
                                                        <button class="modal-close"
                                                            onclick="cerrarModalFalta()">&times;</button>
                                                    </div>
                                                    <form id="formFaltaManual" method="post"
                                                        enctype="multipart/form-data">
                                                        ...
                                                        <div class="modal-body">
                                                            ...
                                                            <div class="form-group">
                                                                <label for="fm_observaciones"
                                                                    class="form-label">Observaciones:</label>
                                                                <textarea id="fm_observaciones" name="observaciones"
                                                                    class="form-textarea"
                                                            ...
                                                            <div class="form-group">
                                                                <label for="fm_foto" class="form-label">Foto
                                                                    (obligatorio):</label>
                                                                <input type="file" id="fm_foto" name="foto"
                                                                    class="form-input" accept="image/*" required>
                                                                <small class="form-text text-muted">Toma una foto de la
                                                                    evidencia (máx. 5MB)</small>
                                                                <img id="fm_foto_preview" class="photo-preview" src="#"
                                                                    alt="Vista previa" style="display: none;">
                                                            </div>
                                                        </div>
