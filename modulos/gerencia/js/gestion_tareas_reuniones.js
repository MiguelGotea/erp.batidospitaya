        ...
        <div class="item-card-row ${claseCard}"
            ...
            : ''}
             ...
             <div class="item-schedule-row">
                ${item.estado === 'finalizado' || item.estado === 'cancelado' ? `
                    <div class="schedule-text-summary">
                        <span class="st-time">${(item.tipo === 'reunion' ? (item.fecha_reunion || '').substring(11, 16) : (item.hora_tarea || '08:00').substring(0, 5))}</span>
                        <span class="st-sep">-</span>
                        <span class="st-time">${calcularHoraFin((item.tipo === 'reunion' ? (item.fecha_reunion || '').substring(11, 16) : (item.hora_tarea || '08:00').substring(0, 5)), (item.duracion_min || 60))}</span>
                        <span class="st-dur">(${item.duracion_min || 60}m)</span>
                    </div>
                ` : `
